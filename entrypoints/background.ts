...
function enableSidePanelActionClick() {
  ...
  // NOTE: We do NOT call setPanelBehavior({ openPanelOnActionClick: true }) here
  // because that overrides default_popup in the manifest and prevents splash.html
  // from showing. The splash.html page opens the side panel itself via postMessage.
}
...
// ── Anubis Agent: multi-provider tab detection ────────────────────────────────
function isAnubisProviderUrl(url: string | undefined): boolean {
  if (!url) return false;
  try {
...
// Extends insertPromptIntoActiveDeepSeekTab to all providers
async function insertPromptIntoActiveProviderTab(
  text: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  if (!text) return { ok: false, error: 'empty_prompt_text' };
  const tabs = await chrome.tabs.query({ currentWindow: true });
  const tab = tabs.find((t) => t.id != null && isAnubisProviderUrl(t.url));
  if (!tab?.id) return { ok: false, error: 'No active AI provider tab found.' };
  try {
    const resp = await chrome.tabs.sendMessage(tab.id, { type: 'INSERT_PROMPT_TEXT', text });
    return resp?.ok ? { ok: true } : { ok: false, error: resp?.error ?? 'Unknown error' };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : String(e) };
  }
}

// ── Auto-detect active provider from real browser tabs ────────────────────
// Removes the need to "Launch" a provider from the panel: simply navigating
// to (or typing the URL of) chatgpt.com / gemini.google.com / chat.deepseek.com /
// claude.ai in ANY tab — and focusing that tab — is enough. We watch tab
// activation + URL changes and keep `anubis_provider` / `anubis_active_tab_id`
// in sync automatically, broadcasting the change so the sidepanel UI can
// switch its selected provider box live, without any manual step.
function urlToProviderId(url: string | undefined): 'deepseek' | 'chatgpt' | 'gemini' | 'claude' | null {
  if (!url) return null;
  try {
    const { hostname } = new URL(url);
...
let lastAutoDetectedProvider: string | null = null;

async function autoSyncActiveProviderFromTab(tab: chrome.tabs.Tab | undefined) {
  if (!tab?.url) return;
  const providerId = urlToProviderId(tab.url);
  if (!providerId) return; // tab isn't an AI provider page — leave selection as-is
  if (providerId === lastAutoDetectedProvider) return;
  lastAutoDetectedProvider = providerId;
  await chrome.storage.local.set({
    anubis_provider: providerId,
    anubis_active_tab_id: tab.id ?? null,
  });
  chrome.runtime.sendMessage({ type: 'ANUBIS_PROVIDER_AUTO_DETECTED', providerId, tabId: tab.id }).catch(() => {});
}

function registerProviderAutoDetection() {
  // Fires when the user switches to a different tab (e.g. alt-tabbing into
  // an already-open ChatGPT/Gemini tab) — selects that provider's box.
  chrome.tabs.onActivated.addListener(async (activeInfo) => {
    try {
      const tab = await chrome.tabs.get(activeInfo.tabId);
      await autoSyncActiveProviderFromTab(tab);
    } catch {
      // tab may have closed mid-query — ignore
    }
  });

  // Fires when the user types/pastes a provider URL into the address bar
  // (or a link navigates a tab there) — no "Launch" button needed.
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (!changeInfo.url) return;
    chrome.tabs.query({ active: true, currentWindow: true }, ([activeTab]) => {
      if (activeTab?.id === tabId) {
        void autoSyncActiveProviderFromTab(tab);
      }
    });
  });

  // Catch the case where the relevant tab is already open/focused when the
  // service worker wakes up (covers cold starts after browser restart).
  chrome.tabs.query({ active: true, currentWindow: true }, ([activeTab]) => {
    void autoSyncActiveProviderFromTab(activeTab);
  });
}

registerProviderAutoDetection();
