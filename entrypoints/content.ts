...
const PET_SPRITE_PATH = 'pet/anubis-pet-states.png';
      ...
      try {
        switch (data.type) {
          ...
          case 'RESPONSE_COMPLETE': {
            ...
            schedulePetIdle();
            // Show response preview in Anubis bubble (chat response bubble from mascot)
            if (currentPetConfig?.enabled && petHostEl?.isConnected && complete.text) {
              const preview = complete.text.replace(/[#*`_~>|]/g, '').trim().slice(0, 120);
              if (preview.length > 8) {
                if (petHostEl) petHostEl.dataset.responseBubble = 'true';
                showPetBubble(preview.length > 100 ? preview.slice(0, 97) + '…' : preview);
                const clearResponseBubble = () => {
                  if (petHostEl) petHostEl.dataset.responseBubble = 'false';
                };
                setTimeout(clearResponseBubble, 7000);
              }
            }
            break;
          }
...
function ensurePet(): HTMLElement {
  ...
  host.style.animation = 'dpp-anubis-entrance 0.85s cubic-bezier(0.22, 1, 0.36, 1) forwards';
  setTimeout(() => { if (host.isConnected) host.style.animation = 'dpp-anubis-gold-shimmer 5s ease-in-out infinite'; }, 900);
...
function injectPetStyles() {
  ...
  style.textContent = `
    #${PET_HOST_ID} {
      ...
      animation: dpp-anubis-gold-shimmer 5s ease-in-out infinite;
    ...
    #${PET_HOST_ID}[data-motion='true'][data-state='thinking'] .dpp-pet-sprite {
      animation: dpp-anubis-think-bob 2.6s ease-in-out infinite;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='speaking'] .dpp-pet-sprite {
      animation: dpp-anubis-speak-pulse 0.85s ease-in-out infinite;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='working'] .dpp-pet-sprite {
      animation: dpp-anubis-work-charge 1.2s ease-in-out infinite;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='confused'] .dpp-pet-sprite {
      animation: dpp-anubis-confused-wobble 2.0s ease-in-out infinite;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='success'] .dpp-pet-sprite {
      animation: dpp-anubis-success-leap 1.3s cubic-bezier(0.22, 1, 0.36, 1) 1;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='error'] .dpp-pet-sprite {
      animation: dpp-anubis-error-rage 0.55s ease-in-out 2;
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='sleepy'] .dpp-pet-motion {
      animation-duration: 8s;
    }
    #${PET_HOST_ID}[data-motion='true'][data-state='sleepy'] .dpp-pet-sprite {
      animation: dpp-anubis-sleepy-drift 3.5s ease-in-out infinite;
    }

    @keyframes dpp-pet-float {
      0%   { transform: translateY(0px) rotate(-0.8deg) scale(1.000); }
      20%  { transform: translateY(-5px) rotate(0.2deg) scale(1.010); }
      45%  { transform: translateY(-11px) rotate(1.2deg) scale(1.018); }
      70%  { transform: translateY(-5px) rotate(0.2deg) scale(1.010); }
      100% { transform: translateY(0px) rotate(-0.8deg) scale(1.000); }
    }

    @keyframes dpp-anubis-entrance {
      0%   { opacity: 0; transform: translateY(40px) scale(0.55) rotate(-6deg); filter: brightness(2); }
      40%  { opacity: 1; transform: translateY(-8px) scale(1.08) rotate(2deg); filter: brightness(1.3); }
      65%  { transform: translateY(3px) scale(0.97) rotate(-1deg); filter: brightness(1); }
      82%  { transform: translateY(-4px) scale(1.03) rotate(0.5deg); }
      100% { transform: translateY(0px) scale(1) rotate(0deg); filter: brightness(1); }
    }

    @keyframes dpp-anubis-hi {
      0%   { transform: rotate(0deg) scale(1) translateY(0); }
      12%  { transform: rotate(-12deg) scale(1.06) translateY(-3px); }
      28%  { transform: rotate(10deg) scale(1.06) translateY(-5px); }
      44%  { transform: rotate(-8deg) scale(1.04) translateY(-3px); }
      60%  { transform: rotate(6deg) scale(1.03) translateY(-2px); }
      76%  { transform: rotate(-3deg) scale(1.01) translateY(-1px); }
      90%  { transform: rotate(2deg) scale(1.01) translateY(0); }
      100% { transform: rotate(0deg) scale(1) translateY(0); }
    }

    @keyframes dpp-anubis-hold {
      0%   { transform: scale(1.15) translateY(-5px) rotate(-2deg); filter: drop-shadow(0 0 18px rgba(218,165,32,0.9)) drop-shadow(0 0 8px rgba(0,230,200,0.5)); }
      30%  { transform: scale(1.18) translateY(-9px) rotate(1.5deg); filter: drop-shadow(0 0 28px rgba(218,165,32,1)) drop-shadow(0 0 14px rgba(0,230,200,0.7)); }
      60%  { transform: scale(1.16) translateY(-6px) rotate(-1deg); }
      100% { transform: scale(1.15) translateY(-5px) rotate(-2deg); filter: drop-shadow(0 0 18px rgba(218,165,32,0.9)); }
    }

    @keyframes dpp-anubis-appear-crown {
      0%   { transform: scaleY(0) translateY(10px); opacity: 0; }
      50%  { transform: scaleY(1.1) translateY(-2px); opacity: 1; }
      100% { transform: scaleY(1) translateY(0); opacity: 1; }
    }

    @keyframes dpp-anubis-orb-pulse {
      0%, 100% { transform: scale(1); opacity: 0.7; }
      50%       { transform: scale(1.4); opacity: 1; }
    }

    @keyframes dpp-anubis-think-bob {
      0%, 100% { transform: translateX(0) rotate(0deg) scale(1); }
      25%  { transform: translateX(-5px) rotate(-2.5deg) scale(0.97); }
      55%  { transform: translateX(3px) rotate(1.5deg) scale(1.02); }
      80%  { transform: translateX(-2px) rotate(-1deg) scale(0.99); }
    }

    @keyframes dpp-anubis-speak-pulse {
      0%, 100% { transform: scale(1) rotate(0deg); }
      20%  { transform: scale(1.055) rotate(0.6deg); }
      40%  { transform: scale(1.07) rotate(-0.6deg); }
      60%  { transform: scale(1.055) rotate(0.4deg); }
      80%  { transform: scale(1.03) rotate(-0.2deg); }
    }

    @keyframes dpp-anubis-work-charge {
      0%   { transform: translateY(0) rotate(0deg) scale(1); filter: brightness(1); }
      15%  { transform: translateY(-7px) rotate(-2deg) scale(1.02); filter: brightness(1.15); }
      35%  { transform: translateY(-4px) rotate(1.5deg) scale(1.04); filter: brightness(1.25); }
      55%  { transform: translateY(-8px) rotate(-1.5deg) scale(1.03); filter: brightness(1.2); }
      75%  { transform: translateY(-3px) rotate(1deg) scale(1.02); filter: brightness(1.12); }
      100% { transform: translateY(0) rotate(0deg) scale(1); filter: brightness(1); }
    }

    @keyframes dpp-anubis-confused-wobble {
      0%   { transform: translateY(0) rotate(0deg) scale(1); }
      18%  { transform: translateY(-4px) rotate(-5deg) scale(0.97); }
      36%  { transform: translateY(-6px) rotate(5deg) scale(1.02); }
      54%  { transform: translateY(-3px) rotate(-4deg) scale(0.98); }
      72%  { transform: translateY(-5px) rotate(3deg) scale(1.01); }
      90%  { transform: translateY(-1px) rotate(-1.5deg) scale(0.99); }
      100% { transform: translateY(0) rotate(0deg) scale(1); }
    }

    @keyframes dpp-anubis-success-leap {
      0%   { transform: scale(0.90) translateY(6px) rotate(-3deg); filter: brightness(0.9); }
      22%  { transform: scale(1.18) translateY(-14px) rotate(4deg); filter: brightness(1.5) drop-shadow(0 0 20px rgba(218,165,32,1)); }
      45%  { transform: scale(1.12) translateY(-8px) rotate(-2deg); filter: brightness(1.3); }
      65%  { transform: scale(1.06) translateY(-4px) rotate(1deg); filter: brightness(1.1); }
      82%  { transform: scale(1.02) translateY(-1px) rotate(-0.5deg); }
      100% { transform: scale(1) translateY(0) rotate(0deg); filter: brightness(1); }
    }

    @keyframes dpp-anubis-error-rage {
      0%   { transform: translateX(0) rotate(0deg) scale(1); filter: brightness(1); }
      10%  { transform: translateX(-8px) rotate(-3deg) scale(1.02); filter: brightness(1.2) hue-rotate(-20deg); }
      22%  { transform: translateX(9px) rotate(3deg) scale(1.03); filter: brightness(1.3) hue-rotate(-30deg); }
      34%  { transform: translateX(-7px) rotate(-2.5deg) scale(1.02); }
      46%  { transform: translateX(7px) rotate(2.5deg) scale(1.01); }
      58%  { transform: translateX(-5px) rotate(-1.5deg) scale(1.01); }
      70%  { transform: translateX(4px) rotate(1deg) scale(1); }
      84%  { transform: translateX(-2px) rotate(-0.5deg); }
      100% { transform: translateX(0) rotate(0deg) scale(1); filter: brightness(1); }
    }

    @keyframes dpp-anubis-sleepy-drift {
      0%   { transform: translateY(0) rotate(-1deg) scale(1); filter: brightness(0.85); }
      35%  { transform: translateY(5px) rotate(-3deg) scale(0.97); filter: brightness(0.75); }
      70%  { transform: translateY(3px) rotate(-1.5deg) scale(0.98); filter: brightness(0.80); }
      100% { transform: translateY(0) rotate(-1deg) scale(1); filter: brightness(0.85); }
    }

    @keyframes dpp-anubis-gold-shimmer {
      0%, 100% { filter: drop-shadow(0 16px 36px rgba(120, 60, 220, 0.40)) drop-shadow(0 2px 10px rgba(218,165,32,0.18)); }
      33%  { filter: drop-shadow(0 16px 36px rgba(80, 30, 180, 0.45)) drop-shadow(0 4px 16px rgba(218,165,32,0.32)); }
      66%  { filter: drop-shadow(0 16px 36px rgba(0, 180, 160, 0.30)) drop-shadow(0 2px 10px rgba(218,165,32,0.22)); }
    }

    @keyframes dpp-bubble-pop {
    ...
    @keyframes dpp-pet-think {
      0%, 100% { transform: translateX(0) rotate(0deg) scale(1); }
      30% { transform: translateX(-4px) rotate(-2deg) scale(0.98); }
    ...
    @keyframes dpp-pet-success {
      0% { transform: scale(0.92) translateY(4px) rotate(-2deg); }
    ...
    #${PET_HOST_ID} .dpp-pet-bubble-text {
      ...
      padding: 5px 11px;
      border-radius: 13px;
      border: 1px solid rgba(140, 80, 255, 0.55);
      background: rgba(255, 255, 255, 0.94);
      -webkit-backdrop-filter: blur(8px);
      backdrop-filter: blur(8px);
      box-shadow: 0 8px 22px rgba(100, 40, 200, 0.22);
      color: #1d2433;
    ...
    #${PET_HOST_ID}[data-response-bubble='true'] .dpp-pet-bubble {
      max-width: 280px;
      bottom: calc(100% - 16px);
    }

    #${PET_HOST_ID}[data-response-bubble='true'] .dpp-pet-bubble-text {
      max-width: 280px;
      white-space: normal;
      font-size: 12px;
      padding: 8px 14px;
      border-radius: 16px;
      border: 1.5px solid rgba(140, 80, 255, 0.6);
      background: linear-gradient(135deg, rgba(255,255,255,0.97) 0%, rgba(240,230,255,0.97) 100%);
      box-shadow: 0 12px 28px rgba(100, 40, 200, 0.22), 0 2px 8px rgba(200, 160, 50, 0.1);
      line-height: 1.5;
    }

    @media (prefers-color-scheme: dark) {
      #${PET_HOST_ID}[data-response-bubble='true'] .dpp-pet-bubble-text {
        background: linear-gradient(135deg, rgba(40,28,65,0.97) 0%, rgba(55,35,80,0.97) 100%);
      }
    }

    #${PET_HOST_ID}[data-dragging='true'] .dpp-pet-sprite {
      filter: brightness(1.2) drop-shadow(0 0 12px rgba(200, 160, 50, 0.6));
    }

    #${PET_HOST_ID}:not([data-dragging='true']) .dpp-pet-motion {
      transition: transform 0.3s cubic-bezier(0.34, 1.2, 0.64, 1);
    }

    #${PET_HOST_ID}[data-motion='true'][data-state='idle'] .dpp-pet-sprite {
      animation: dpp-anubis-hi 3.2s cubic-bezier(0.34, 1.1, 0.64, 1) 1;
      ...
      filter: drop-shadow(0 2px 2px rgba(100, 40, 200, 0.15));
    }
    ...
    @media (prefers-color-scheme: dark) {
      #${PET_HOST_ID} .dpp-pet-bubble-text {
        ...
        color: #eef2ff;
        box-shadow: 0 8px 22px rgba(80, 20, 160, 0.45);
      }
