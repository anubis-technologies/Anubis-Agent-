...
export const zhCN = {
  ...
  locale: {
    settingTitle: '语言',
    settingDescription: '选择 Anubis Agent 的界面和模型行为语言。',
    auto: '跟随浏览器',
    zhCN: '简体中文',
    en: 'English',
  },
  sidepanel: {
    ...
    localSkillImport: {
      title: '从本地导入 Skill',
      description: '通过 Shell MCP 读取本地 Skill 文件夹。Anubis Agent 会保留文件夹引用，导入时不会执行脚本。',
    ...
    whatsNew: {
      ...
      dismiss: '知道了',
      items: {
        browserControl: '浏览器控制可在侧边栏选择目标标签页，让 Anubis Agent 执行可见网页操作。',
    ...
    memoryPage: {
      title: '记忆',
      description: '管理会长期影响 Anubis Agent 回复的用户偏好、反馈、话题背景和参考资料。',
      emptyAll: '暂无记忆，对话时会自动积累',
      emptyFiltered: '该分类下暂无记忆',
      emptyHelp: '对话中出现稳定偏好或项目背景时，Anubis Agent 会自动保存；也可以手动新增一条。',
      count: '共 {count} 条记忆',
    ...
    settings: {
      ...
      petWhale: 'Anubis 守护神',
      petWhaleDescription: '在 DeepSeek 页面显示 Anubis 守护神，附带对话气泡',
    ...
    promptControls: {
      ...
      systemPrompt: 'Anubis Agent 系统提示词',
    ...
    browserControlPage: {
      title: '浏览器控制',
      description: '让 Anubis Agent 通过浏览器 Debugger 和 Accessibility Tree 控制选中的 Chrome 或 Edge 标签页。',
      enableTitle: '启用浏览器工具',
    ...
    mcpPage: {
      title: 'MCP',
      description: '连接本机或远程 MCP 服务，让 Anubis Agent 可以发现工具、控制自动执行策略并回传结果。',
    ...
    chatPage: {
      ...
      inputPlaceholder: '给 Anubis Agent 发送消息',
      send: '发送',
      sendFailed: '发送失败',
      uploadImage: '上传图片',
  ...
  content: {
    ...
    historyOrganizer: {
      enhancedSearchTitle: 'Anubis Agent 搜索增强',
      tagFilterLabel: '按标签筛选结果',
      tagPlaceholder: '输入标签名',
      currentTagsLabel: '给当前对话加标签',
      currentTagsPlaceholder: '逗号分隔，例如：港股, 写作',
      emptySearchStatus: 'Anubis Agent：等待官方搜索结果',
      visibleStatus: 'Anubis Agent：已显示 {visible}/{total}',
      loadError: 'Anubis Agent：历史标签加载失败：{message}',
      saveError: 'Anubis Agent：历史标签保存失败：{message}',
    },
    projectSidebar: {
      title: '项目',
      empty: '还没有项目',
    ...
    toolBlock: {
      ...
      missingSandboxResultDetail: '沙箱工具没有返回执行结果。请重载 Anubis Agent 扩展并刷新当前 DeepSeek 页面后重试；它不依赖 MCP 或 Shell Local。',
    ...
    agent: {
      ...
      budgetReached: 'Anubis Agent 已执行 {count} 轮自动工具续跑并暂停，避免把未完成任务伪装成最终答案。请发送“继续”让当前对话接着处理剩余工作。',
      error: '执行出错',
      footerComplete: 'Agent 完成（{steps} 步，{tools} 次工具调用）',
      footerError: 'Agent 执行出错（{steps} 步，{tools} 次工具调用）',
    },
    permission: {
      webFetch: 'Anubis Agent 需要访问 {origin} 的权限以获取该页面内容',
      deny: '拒绝',
      grant: '授权',
      requesting: '请求中...',
    },
    ...
    multimodalMedia: {
      buttonTitle: '添加 Anubis Agent 多模态图片或视频',
      removeTitle: '移除 {name}',
  ...
  tool: {
    ...
    web: {
      ...
      searchFailed: '搜索失败',
      permissionDenied: '扩展没有访问必应的权限。请完全移除扩展后重新加载 dist/chrome-mv3 目录，或在 chrome://extensions → Anubis Agent 详情中确认 cn.bing.com 已列入网站访问权限。',
    ...
    browser: {
      providerName: 'Anubis Agent 浏览器控制',
      navigateTitle: '打开网页',
