<p align="center">
  <img src="assets/readme-header.png" width="860" alt="Anubis Agent DeepSeek 浏览器插件和 AI Agent 工作台">
</p>

<h1 align="center">Anubis Agent</h1>

<p align="center">
  <strong>Anubis Agent：把 DeepSeek 网页版扩展成支持中英文体验、记忆、项目、Skill、MCP、多模态媒体、浏览器控制、保存项、产物下载、对话导出和自动化的 AI Agent 工作台</strong>
</p>
...
Anubis Agent 是面向 [DeepSeek](https://chat.deepseek.com) 网页版的开源浏览器扩展，支持 Chrome、Edge 和 Firefox。它把 DeepSeek Web 扩展成 AI agent workspace，让用户在同一浏览器工作流里使用中英文界面、MCP 工具、图片/视频多模态分析、长期记忆、Skill、系统提示词预设、联网搜索、网页读取、对话导出和定时自动化。

如果你在寻找 DeepSeek Chrome extension、DeepSeek MCP tools、DeepSeek memory plugin、DeepSeek conversation export 或 DeepSeek AI agent，Anubis Agent 对应的是同一个本地优先的 DeepSeek 浏览器增强工作台。

语言可设为跟随浏览器、简体中文或 English。Anubis Agent 会让侧边栏、右键菜单、工具结果、内置 Skill 行为和自动续跑提示保持一致语言，同时保留用户自己写的记忆、预设、自定义 Skill、自动化任务和同步数据原文。
...
| DeepSeek multimodal media / 图片视频分析 | 可在侧边栏网页登录对话的识图模式附加图片；安装多模态 Native Host 后，也可在 DeepSeek 输入框附加图片或视频，让 Anubis Agent 先完成媒体分析再带着结果继续对话。 |
| DeepSeek browser control / 浏览器控制 | 在侧边栏选择受控标签页，让 Anubis Agent 按用户开启的边界读取页面结构并执行可见网页操作。 |
...
- 希望 Anubis Agent 的界面、工具提示和模型续跑提示能跟随中文或英文使用环境。
...
- **单文件产物** — 让 Anubis Agent 生成脚本、Markdown、JSON、HTML 等可下载文件
- **项目包产物** — 多文件结果可打包下载，适合原型、小工具或文档集合
- **本地优先** — 项目上下文、项目记忆和生成产物由用户主动维护和下载，不需要 Anubis Agent 托管后台
...
- **本地保存** — 导出文件通过浏览器本地下载保存，Anubis Agent 不运营用于收集导出数据的后台服务
...
| 方向 | 主要变化 |
|------|----------|
| 用量与速度可见性 | 设置页新增用量统计入口，可查看 token、请求、耗时和速度等本地统计，帮助用户理解当前 Anubis Agent 会话的执行节奏。 |
...
1.0.0 是多模态媒体与侧边栏工作台成熟版本，重点让 Anubis Agent 可以把图片、视频、MCP、本机工具和更清晰的管理界面放进同一条浏览器任务流。
...
| 方向 | 主要变化 |
|------|----------|
| 本地 Skill 管理 | Skill 页新增本机 Skill 预览、导入和同步能力，可把个人或团队本地 Skill 文件夹纳入 Anubis Agent 工作流。 |
...
0.7.2 是浏览器控制和侧边栏体验增强版本，重点让 Anubis Agent 可以在用户选定的标签页中执行可见网页操作，同时把新权限、第三方 Skill 管理和侧边栏状态反馈讲清楚。
...
0.7.0 的主线是把 Anubis Agent 从 DeepSeek 网页增强，推进到可持续工作的浏览器内 AI Agent 工作台：上下文可以沉淀，输出可以下载，工具执行可以被用户审查、接管和复盘。
...
0.6.5 是中英文运行体验和输出反馈校准版本，重点让 Anubis Agent 在中文或英文环境下保持一致的操作语言，并让实时输出速度显示更贴近实际生成状态。
...
0.6.4 是侧边栏对话和本机工具增强版本，重点让普通网页里的 Anubis Agent 对话入口更独立，也让 Shell MCP 支持 Python 解释器工作流。
...
0.6.0 是侧边栏对话和 Skill 工作流增强版本，重点让 Anubis Agent 从增强网页对话，推进到可在侧边栏直接发起任务、管理自定义 Skill 并从 GitHub 导入 Skill。
...
0.2.0 汇总了 0.1.0 以来的主要增量，重点是把 Anubis Agent 从“记忆 + Skill”扩展升级为完整的浏览器端工具平台。
...
Chrome 用户可以直接从 [Chrome Web Store](https://chromewebstore.google.com/detail/deepseek++/kdmpkkahkhdmdhfkdihkopikgcocbpbf?hl=zh-CN) 安装 Anubis Agent。安装后打开 [DeepSeek 网页版](https://chat.deepseek.com)，即可在侧边栏中按需启用记忆、Skill、MCP、联网工具、对话导出和自动化能力。
...
仓库包含 Android WebView 基线，用于验证 Anubis Agent 的移动端能力边界。当前可运行：
