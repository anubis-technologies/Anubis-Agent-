<p align="center">
  <img src="assets/readme-header.png" width="860" alt="Anubis Agent DeepSeek browser extension and AI agent workspace">
</p>
...
Language can follow the browser or be set to English or Simplified Chinese. Anubis Agent keeps the side panel, context menus, tool results, built-in Skill behavior, and continuation prompts in the selected language while preserving user-authored memories, presets, custom Skills, automation tasks, and sync data as written.
...
| Need | What Anubis Agent provides |
|------|--------------------------|
...
| DeepSeek multimodal media | Attach images in Vision mode from side-panel web chat; after installing the Multimodal Native Host, also attach images or videos in the DeepSeek input box so Anubis Agent can analyze the media first and continue the conversation with the results. |
| DeepSeek browser control | Lets Anubis Agent operate a user-selected browser tab after the user enables the feature and chooses the target. |
...
- Use Anubis Agent in an English or Simplified Chinese workflow with matching UI, tool guidance, and model continuation prompts.
...
- **Single-file artifacts** - Ask Anubis Agent to create downloadable scripts, Markdown, JSON, HTML, or other text files.
- **Project bundles** - Download multi-file results as a bundle for prototypes, small tools, or documentation sets.
- **Local-first flow** - Project context, project memories, and generated artifacts are maintained and downloaded by the user without a Anubis Agent backend.
...
- **Local saves** - Export files are saved through the browser's local download flow. Anubis Agent does not operate a backend for collecting export data.
...
- **Local import** - Preview, import, and sync local Skill folders so personal workflows can be reused in Anubis Agent.
...
| Area | Main changes |
|------|--------------|
| Usage and speed visibility | Adds a Usage settings page for local token, request, duration, and speed statistics so users can understand how the current Anubis Agent workflow is running. |
...
1.0.0 matures Anubis Agent as a multimodal browser workspace, bringing image/video analysis, MCP, local tools, and cleaner side-panel management into the same task flow.
...
| Area | Main changes |
|------|--------------|
| Local Skill management | Adds local Skill preview, import, and sync support in the Skills page so personal or team Skill folders can join the Anubis Agent workflow. |
...
0.7.2 adds Browser Control and tightens the side-panel experience, so Anubis Agent can operate a user-selected tab while keeping permission boundaries, third-party Skill management, and user feedback clearer.
...
0.7.0 moves Anubis Agent from a DeepSeek web-page enhancement toward a sustained browser-based AI agent workspace: context can be reused, outputs can become downloadable files, and tool execution can be reviewed, steered, and resumed.
...
0.6.5 focuses on bilingual runtime polish and output feedback accuracy. Anubis Agent now keeps Chinese or English workflows more consistent across the extension while making the live output-speed indicator better match actual generation.
...
0.6.4 strengthens side-panel chat and local tool workflows. It makes Anubis Agent chat usable on normal web pages with an official API Key and adds Python interpreter support through Shell MCP.
...
0.6.0 focuses on side-panel chat and Skill workflow improvements. Anubis Agent now moves beyond enhancing the page conversation: it can start tasks directly from the side panel, manage custom Skills, and import Skills from GitHub.
...
0.2.0 collects the major additions since 0.1.0 and upgrades Anubis Agent from "memory + Skills" into a browser-side tool platform.
...
The repository includes an Android WebView baseline for validating Anubis Agent mobile capability boundaries. The available local staging command is:
